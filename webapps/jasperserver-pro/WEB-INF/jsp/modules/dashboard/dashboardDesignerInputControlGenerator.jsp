...
<cip:set var="controlType" value="${control.inputControlType}"/>

<cip:if test="${control.visible}">
<cip:choose>
<%-- ##########   BOOLEAN   ########## --%>
<%-- InputControl.TYPE_BOOLEAN --%>
<cip:when test="${control.inputControlType == 1}">
    <!--dummy-->
...
<cip:when test="${control.inputControlType == 2}">
    <cip:set var="frameWidth" value="${frame.width <= 1 ? 100 : frame.width}px"/>
    <cip:choose>
        <cip:when test="${control.dataType.localResource.dataTypeType == 3 || control.dataType.localResource.dataTypeType == 4}">
            <script type="text/javascript">
                var calendarDateFormat = "<spring:message code='calendar.date.format'/>";
            </script>
            <cip:set var="frameClass" value="picker"/>
        </cip:when>
        <cip:otherwise>
            <cip:set var="frameClass" value="text"/>
        </cip:otherwise>
    </cip:choose>
    <!--dummy-->
    <div id="controlFrameContainer_${frame.name}" class="componentContainer control ${frameClass}"
        ...
        <div id="controlFrameOverlay_${frame.name}"
             ...
             data-controlType="${controlType}"
             data-isReadOnly="${readOnly}"
                <cip:if test="${control.dataType.localResource.dataTypeType == 4}">
                    data-calTime="true"
                </cip:if>
...
<%-- ##########   SINGLE_SELECT   ########## --%>
<cip:when test="${(control.inputControlType == 3) || (control.inputControlType == 4)}">
    <cip:choose>
...
<cip:when test="${(control.inputControlType == 6) || (control.inputControlType == 7)}">
    <cip:set var="frameHeight" value="${(frame.height) < 150 ? 150 : frame.height}"/>
    <cip:set var="frameWidth" value="${frame.width < 100 ? 100 : frame.width}"/>
    <!--dummy-->
    <div id="controlFrameContainer_${frame.name}"  class="componentContainer control select multiple"
         style="left:${frameLeft};top:${frameTop};height:${frameHeight}px;width:${frameWidth}px;z-index:10;">
...
<cip:when test="${(control.inputControlType == 8) || (control.inputControlType == 9)}">
...
<cip:when test="${(control.inputControlType == 10) || (control.inputControlType == 11)}">
