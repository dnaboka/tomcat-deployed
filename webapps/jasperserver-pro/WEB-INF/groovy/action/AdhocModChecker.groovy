...
import com.jaspersoft.ji.adhoc.AdhocUnifiedState
 ...
 * </p>
 * Using the modFlags property of the state to communicate a type of state modification.
 * Normal flow:
 * <ol>
 * <li>State starts out with empty modFlags</li>
 * <li>After each AJAX request, the action name is added to modFlags (space-separated set of actions)</li>
 * <li>On checkSave action, typeOfViewChange is set based on whether modFlags contains any of the actions in various action sets (unexpectedBehaviorActions, etc.)</li>
 * <li>On save, modFlags is cleared</li>
 * </ol>
 */
class AdhocModChecker implements RequestMonitor {
	...
	def getSavedView(req) {
		...
		def savedView 
		if (currentView) {
			savedView = engine.getRepositoryService().getResource(null, currentView.URIString)
		} 
		return savedView
	}
	...
	*/
	...
	// 
	def getModFlags(req) {
		def modFlagString = getState(req).getProperty(AdhocUnifiedState.MOD_FLAGS)
		(modFlagString ? modFlagString.split(" ") : []) as Set
	}
	
	def setModFlags(req, flags) {
		getState(req).setProperty(AdhocUnifiedState.MOD_FLAGS, flags.join(" "))
	}
