...
//  boolean isAnyValue(Boolean bool) { bool != null}
    ...
    def toTime(obj) {
        ...
        //      2016-01-13  thorick
        //
        // note that when to convert java.sql.Timestamp to java.sql.Time
        // you lose the fractional second accuracy of Timestamp
        // in our conversion here we ignore all nanoseconds and defer to the JVM
        // to handle nanosecond AND millsecond rounding.
        //
    ...
    def toTimestamp(obj) {
        if (obj instanceof Timestamp) {
            ...
            return new java.sql.Timestamp(((java.sql.Time)obj).getTime());
    ...
    */
	...
	// FIXME these should return null if any operands are null, which is what you get from a relational DB.

    def add(long[] args) {
        BigInteger total = 0;
        for (long arg : args) { if (arg != null) { total += arg } }
        return total
    }

    def add(double[] args) { double total = 0; for (double arg : args) { if (arg != null) total += arg }; return total }

    def add(Object[] args) { BigDecimal total = 0; for (BigDecimal arg : args) { if (arg != null) total += arg }; return total }

    def subtract(arg1, arg2) { (arg1 ?: 0) - (arg2 ?: 0) }

    def multiply(long[] args) { long total = 1; for (long arg : args) { if (arg != null) total *= arg }; return total }

    def multiply(double[] args) { double total = 1; for (double arg : args) { total *= (arg ?: 0)  }; return total }

    def multiply(Object[] args) { BigDecimal total = 1; for (BigDecimal arg : args) { total *= (arg ?: 0)  }; return total }
...
*/
    ...
    def customDoublesContains(arg1, list, truncatedList) {
        ...
        if ( (list.contains(arg1) || ((arg1 != null) && truncatedList.contains(truncatingEpsilon(arg1)))))
        {
            return true;
        }
        // we almost need to check if numbers are close enough
        return checkDoubleContains(list, arg1) || checkDoubleContains(truncatedList, arg1)
