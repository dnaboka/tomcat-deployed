...
dd.tables = {
    ...
    getSelectedModel: function() {
        ...
        var rootNode = this.destinationTablesTree.getRootNode();

        domain.unescapeTree(rootNode);

        return Object.toJSON(rootNode.childs.findAll(function(node) {
    ...
    fillForm: function() {
        ...
        $('datasources').writeAttribute('value', Object.toJSON([this.dataSource]));
    ...
    updateDataSourceLabel: function(label) {
        $(this.DATA_SOURCE_LABEL_DOM_ID).update(xssUtil.escape(label));
    },
    ...
    checkForSchemas: function(dataSource, callback) {
        ...
        var postData = {
            selectedDatasources: Object.toJSON([dataSource])
        };
    ...
    pickDataSource: function(replaced) {
        ...
        if (tree && tree.getRootNode()) {
            domain.unescapeTree(tree.getRootNode());
            selectedModel = Object.toJSON(tree.getRootNode().childs.findAll(function(node) {
                return !node.isHidden();
            }));
        }
