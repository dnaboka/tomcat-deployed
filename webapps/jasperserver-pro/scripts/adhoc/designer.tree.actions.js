...
/* global dynamicTree, localContext */
...
define(function(require) {
    ...
    /**
     * Add a new field node to the available fields tree.
     * @param metaNode The node being added
     * @param tree The destination tree
     */
    adhocDesigner.addNodeToTree = function(metaNode, tree) {
        //create new node
        var newNode = tree.processNode(metaNode);
        newNode.isloaded = true;
        newNode.editable = false;
    
        tree.rootNode.addChild(newNode);
        newNode.refreshStyle();
    }
    
    
    /**
     * Remove the field node from the fields tree.
     * @param metaNode The node being removed
     */
    adhocDesigner.removeNodeFromTree = function(metaNode) {
        if (metaNode.parent) {
            var parentNode = metaNode.parent;
            var isMeasure = metaNode.param.extra.isMeasure;
            ...
            throw("The parent node is undefined");
        }
    };
