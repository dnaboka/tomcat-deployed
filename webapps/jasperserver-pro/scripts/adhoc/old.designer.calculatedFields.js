        ...
        if($("calculatedField")){
            ...
            $("calculatedField").observe('keypress', function(evt){
                ...
                if(element.match("input")){
                    ...
                    var proceed = this.checkValidDecimalInput(evt);
                    if(!proceed){
                        Event.stop(evt);
                    }
                }
...
/**
    ...
    render: function (state, container) {
        if (state && !_.isEmpty(state)) {
            var templateFn = adhocDesigner.registerTemplate(adhocCalculatedFields, "calcFieldsTemplate");
            jQuery(container).html(templateFn(state));
        }
    },
    ...
    checkValidDecimalInput:function (evt) {
        var character = null;
        var element = evt.element();
