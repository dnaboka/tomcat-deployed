...
//            this.tree.showTreePrefetchNodes(uriList.join(",")); //Commented becouse of issue with custom fields
        ...
        designerBase.sendRequest(adhocDesigner.getControllerPrefix() + "_updateAvailableTree", [], callback, {'target':"ajaxbuffer", 'bPost' :true});
    },
    ...
    createAvailableFieldsTree: function(uriList){
        ...
        tree.observe('tree:loaded', function(event) {
            _.each(adhocDesigner.getAllLeaves(null, tree),function(leaf) {
                var field = adhocDesigner.findFieldByName(leaf.param.id);
                if (!!field.aggregateField) {
                    adhocDesigner.removeNodeFromTree(leaf);
                }
            });
