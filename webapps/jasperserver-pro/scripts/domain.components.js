...
//    } else if (this.hasChilds() && disabledChildren > 0 && disabledChildren == visibleChildren) {
    } else if (this.getType() === this.Types.Leaf) {
        this.deselect();
...
domain.ItemNode.addMethod('toJSON', function() {
    ...
    if (extra) {
        json.labelId = extra.labelId || "";
        json.descr = extra.descr;
        json.descrId = extra.descrId || "";
        json.dataType = extra.dataType || "";
        json.resourceId = extra.resourceId;
    }
    if (this.isParent()) {
        json.children = _.chain(this.childs).
            filter(function(c) {
                return !c.isHidden();
            }).
            invoke("toJSON").
...
domain.TwoColumnNode.addMethod('_createNode', function() {
    ...
    var origLabel =
            this.param.extra && this.param.extra.label
                    ...
                    : this.name;
    columnOne.insert(xssUtil.escape(origLabel));

    var columnTwo = wrapper.childElements()[2];
    columnTwo.insert(xssUtil.escape(this.name));
...
domain.TwoColumnNode.addMethod('toJSON', function() {
    ...
    var json = {
        ...
        label : this.name,
        type : this.param.type,
        order : this.orderNumber.toString()
    };
    if (extra) {
        json.labelId = extra.labelId || "";
        json.dataType = extra.dataType || "";
    ...
    if (this.isParent()) {
        json.children = _.invoke(this.childs, "toJSON");
    }
    return json;
});
...
domain.TablesNode.addMethod('toJSON', function() {
    ...
    if (extra) {
        json.originalId = extra.originalId;
        json.datasourceId = extra.datasourceId;
        json.repoId = extra.repoId;
        json.itemId = extra.itemId;
        json.dataSource = extra.dataSource;
        json.table = extra.table;

        if (this.isParent()) {
            json.query = extra.query;
            json.dataSetType = extra.dataSetType;
        } else {
            json.expression = extra.expression;
            json.islandId = extra.islandId;
            json.isConstant = extra.isConstant;
            json.JavaType = extra.JavaType;
            json.JdbcType = extra.JdbcType;
        }

        if (extra.fieldDbName) {
            json.fieldDbName = extra.fieldDbName;
        }
...
domain.TablesCopyMoveController.addMethod('_buildMetanode', function(node) {
    ...
    if (node.param.extra) {
        ...
        if (node.param.extra.fieldDbName) {
            metanode.extra.fieldDbName = node.param.extra.fieldDbName;
        }
...
domain.JoinsListItem.addMethod('processTemplate', function(element) {
    ...
    columnOne.update(xssUtil.escape(this._value.left.table.label) + ':' + xssUtil.escape(this._value.left.label));
    columnTwo.update(xssUtil.escape(this._value.right.table.label) + ':' + xssUtil.escape(this._value.right.label));
...
domain.DisplayNode.addMethod('toJSON', function() {
    ...
    if (extra && extra.label) {
        json.label = extra.label
    }
    if (this.isParent()) {
        json.children = _.chain(this.childs).
            filter(function(c) {
                return !c.isHidden() && !c._markAsDeleted;
            }).
            invoke("toJSON").
            value();
    } else {
        json.dataType = extra.dataType;
        json.dimensionOrMeasure = extra.dimensionOrMeasure;
        json.defaultMask = extra.defaultMask;
        json.defaultAgg = extra.defaultAgg;
    }
    return json;
});
