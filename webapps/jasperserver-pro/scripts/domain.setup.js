...
domain.wizard = {
    ...
    initialize: function() {
        ...
        // Prototype.js cannot stop IE10 from submitting of this form
        jQuery("#"+this.STEP_DISPLAY_FORM).on('submit', function(e) {
            e.preventDefault();
        });
        ...
        function _initBundleList() {
            ...
            items.each(function(item) {
                item.processTemplate = function(element) {
                    ...
                    element.select('p.one > a')[0].update(xssUtil.escape(this._value));
                    element.select('p.two > a')[0].update(xssUtil.escape(domain.getMessage("CSLD_CHANGE_LINK")));
                    element.select('p.two > a')[1].update(xssUtil.escape(domain.getMessage("CSLD_REMOVE_LINK")));
...
//////////////////////
    ...
    checkDataSource: function(ds, callback) {
        ...
        var params = {
            datasources: "['" + xssUtil.escape(ds.uri) + "']"
        };
...
/////////////////////////////////////////
    ...
    getBundleSuffix: function(fileName) {
        var parser = new RegExp(".*(?:(?:[_])([a-z][a-z]))(?:[_]){0,1}([A-Z][A-Z]){0,1}(?:(?:[_])([^_]*)){0,1}\.properties"); // jshint ignore: line
...
//////////////////
    ...
    validateAndSave: function(validateUploadedShema) {
        ...
        var validateCallback = function(response) {
            if (response.result) {
                ...
                // validation failed
                if (validateUploadedShema) {
                    if (!domain.wizard.isSchemaSet){
                        domain.enableButton(this.BUTTON_SAVE, false);
                        $(this.LAUNCH_DD).update(domain.getMessage("CSLD_CREATE_WITH_DD"));
                    }
                    dialogs.systemConfirm.show(domain.getMessage('CSLD_SCHEMA_UPLOAD_ERROR'), 5000);
                }
                ...
                if (!response.message) {
                    msg = domain.getMessage('CSLD_DOMAIN_VALIDATION_ERROR');
                } else {
                    usedResources = domainUtil.getUsedResourcesList(response.message, domain._messages['ITEM_BEING_USED_BY_RESOURCE']);
                    msg = (usedResources.length || domainUtil.indexOfNoAccessMessage(response.message, domain._messages['resourcesWithNoAccess']) > -1)
                        ? domainUtil.getUsedResourcesFormattedMessage(usedResources, response.message, {
                ...
                if (domain.wizard.allowDesignerCloseWithValidationErrors && (usedResources.length || domainUtil.indexOfNoAccessMessage(response.message, domain._messages['resourcesWithNoAccess']) > -1)) {
                    domain.confirmDialog.show(
                        "invalidDesignConfirmMessage",
                        _.bind(function() { this._doSave(false); }, this),
                        function() {},
                        _.bind(function(element) { this._showAffectedResources(element, msg); }, this),
                        domain.confirmDialog.MODE_YES_NO
                    );
                } else {
                    domain.detailsDialog.show(msg);
                }
            }
        }.bind(this);
    ...
    _showAffectedResources: function(element, errorMessage) {
        if (domain.elementClicked(element, "#designDetails")) {
    ...
    _doSave: function(overwrite) {
        ...
        var saveCallback = function(response) {
            if (response.result) {
                ...
                setTimeout(this.exit,500); // fixed bug #30339
            } else {
...
////////////////////
    ...
    _uploadBundle: function(dialog,options) {
        ...
        function endUploadCallback(response) {
            ...
            if (response.result) {
                if (options.oldUri) {
                    ...
                    item.processTemplate = function(element) {
                        element.select('p.one > a')[0].update(xssUtil.escape(this._value));
                        element.select('p.two > a')[0].update(xssUtil.escape(domain.getMessage("DOMAIN_WIZARD_CHANGE_DATASOURCE")));
                        element.select('p.two > a')[1].update(xssUtil.escape(domain.getMessage("DOMAIN_WIZARD_REMOVE_DATASOURCE")));
                        return element;
                    };
        ...
        var errorElement, fromRepo = dialog.filePath.hasAttribute(layoutModule.DISABLED_ATTR_NAME);
        ...
        if (!errorMessage) {
            ...
            if (bundleExists) {
				// highlight error
				errorElement = fromRepo ? dialog.fromRepoRadio : dialog.filePath;
