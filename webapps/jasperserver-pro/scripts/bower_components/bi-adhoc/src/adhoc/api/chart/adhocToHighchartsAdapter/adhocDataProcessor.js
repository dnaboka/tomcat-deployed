...
(function (factory, globalScope) {
    ...
    if (typeof define === "function" && define.amd) {
        ...
        // Browser globals.
        globalScope.AdhocDataProcessor = factory(_, xssUtil); // jshint ignore: line
    }
}(function (_, xssUtil) {
    ...
    "use strict";
     ...
     var AdhocDataProcessor = {
            ...
            fn: {
                ...
                getMeasureLevelNumber: function (metadata) {
                    if (!metadata) {
                        metadata = this.metadata;
                    }

                    var levels = metadata.axes[metadata.measureAxis];
                    for (var i = 0; i < levels.length; i++) {
                        if (levels[i].dimension == 'Measures') {
                            return (i + 1);
                        }
                    }
                },
                ...
                getDataFromRowColumn: function (rowLeaf, columnLeaf) {
                    ...
                    var data =  AdhocDataProcessor.data[rowLeaf.axisCoordinate][columnLeaf.axisCoordinate];

                    if (typeof data === "string") {
                        data = (data === "" ? null : parseFloat(data));
                    }
                ...
                getLevelsWithoutMeasures: function (levels) {
                    return _.chain(levels)
                        ...
                        .map(function(level){
                            level.name = xssUtil.unescape(level.name); // jshint ignore: line
                            level.label = xssUtil.unescape(level.label); // jshint ignore: line
                            return level;
                        })
                        .value();
                },
    ...
    return {
        ...
        getMeasureLevelNumber: AdhocDataProcessor.fn.getMeasureLevelNumber
    };
}, this));