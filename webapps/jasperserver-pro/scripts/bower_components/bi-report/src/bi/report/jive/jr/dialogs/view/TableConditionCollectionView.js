...
define(function (require) {
    ...
    var Backbone = require("backbone"),
        ...
        tableConditionCollectionTemplate = require("text!../template/tableConditionCollectionTemplate.htm"),
        TableConditionView = require("./TableConditionView"),
        TableConditionModel = require("../model/TableConditionModel");

    function onApplyToChange(viewModel, value, options) {
        var cache = this.cache,
            currentApplyTo = value,
            previousApplyTo = viewModel.previous("applyTo"),
            columnComponentModel = this.columnComponentModel,
            key, keyModel,
            columnGroup;

        if (currentApplyTo == null || options.isReset) { return; }
        ...
        if (currentApplyTo === "detailrows") {
            ...
            if (cache.get(key) === null) {
                cache.set(key, schemaConditionsToJiveConditions.call(
                    this,
                    columnComponentModel.conditions.toJSON(),
                    columnComponentModel.get("dataType")
                ));
            }
        } else {
            ...
            if (cache.get(key) === null) {
                cache.set(key, schemaConditionsToJiveConditions.call(
                    this,
                    columnGroup.conditions.toJSON(),
                    columnGroup.get("dataType")
                ));
            }
        }
    ...
    function schemaConditionsToJiveConditions(schemaConditions, jiveDataType) {
        var self = this,
            schemaDataType = jiveDataConverter.dataTypeToSchemaFormat[jiveDataType],
            genericProperties;
    ...
    function jiveConditionsToSchemaConditions(jiveConditions, jiveDataType, conditionPattern) {
        ...
        return jiveConditions.map(function(condition) {
            if (condition.operator) {
                var valueStart = _.isArray(condition.value) ? condition.value[0] : condition.value,
                    valueEnd = _.isArray(condition.value) ? condition.value[1] : undefined;

                schemaOpVal = jiveDataConverter.operatorAndValueToSchemaFormat.call(self.columnComponentModel,
                    condition.operator, jiveDataConverter.dataTypeToSchemaFormat[jiveDataType], valueStart, valueEnd, conditionPattern);
            }
    ...
    var TableConditionCollection = Backbone.Collection.extend({
        model: TableConditionModel
    });
    ...
    return Epoxy.View.extend({
        events: {
            "click .jive_inputbutton[name='conditionAdd']": "addNewCondition"
        },
        ...
        addSubview: function(model) {
            var subview = new TableConditionView(_.extend({}, { i18n: this.i18n, model: model, genericProperties: this.columnComponentModel.parent.config.genericProperties }));
        ...
        addModel: function(modelJSON, skipRegistration) {
            var subview = new TableConditionView({i18n: this.i18n, genericProperties: this.columnComponentModel.parent.config.genericProperties }),
                applyTo = this.viewModel.get("applyTo"),
                dataType, model;
            ...
            subview.viewModel.set({
                conditionOptions: this.columnComponentModel.parent.config.genericProperties.operators[dataType],
                dataType: dataType,
        ...
        addAll: function(arrJsonModels) {
            ...
            this.collection.set(_.map(arrJsonModels, function(modelJSON, index) {
                var model = self.addModel(modelJSON, true),
                    view = self.getSubviewByModel(model);
        ...
        addNewCondition: function(evt) {
            ...
            var
                tableConditionModel = new TableConditionModel(),
                model = this.addModel(tableConditionModel.defaults()),
                view = this.getSubviewByModel(model);
        ...
        getActions: function() {
            ...
            _.each(cache.map, function(cacheEntry, key) {
                var keyInfo = cache.keyInfo[key];

                // FIXME: check if collection has changed before adding it to action
                keyInfo.model.updateFromReportComponentObject({
                    conditions: jiveConditionsToSchemaConditions.call(self, cacheEntry.current, keyInfo.model.get("dataType"), keyInfo.model.get("conditionalFormattingData").conditionPattern)
                });
