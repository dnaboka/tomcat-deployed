	...
	return Backbone.View.extend({
		...
		show: function() {
			...
			if (this.$biComponentContainer.length < 1) {
				log.debug("loading overlay: external container is absent");
				return;
			}
			...
			if (this.$biComponentContainer.height() === 0 || this.$biComponentContainer.width() === 0) {
				log.debug("loading overlay: component container has zero size");
				return;
			}

			if (this.propertiesModel.get("isolateDom")) {
				if (!this.$biComponent.height()) {
					log.debug("iframe mode: bi component hasn't any size");
					return;
				}
			}
		...
		_attachToContainer: function() {
			...
			this.$overlayHolder = $("<div></div>").append(
				$("<div></div>").css({
					width: "100%",
					height: "100%",
			...
			this.$overlayHolder.append(this.overlayDOMObject);
        ...
        _adjustOverlaySizeAndPosition: function () {
            ...
            if (exContainer.scrollWidth <= exContainer.outerWidth) {
                ...
                log.debug("width: biComponent is smaller than container or equal OR container doesn't have horizontal scroll bars");
                ...
                log.debug("width: biComponent is larger than container");

                left = exContainer.scrollLeft -
                    (exContainer.scrollLeft < exContainer.padding.left ? 0 : exContainer.padding.left);
                ...
                if (exContainer.scrollLeft + exContainer.outerWidth >= exContainer.scrollWidth - exContainer.padding.right) {
                    ...
                    log.debug("width: scrolled to the end");
            ...
            if (exContainer.scrollHeight <= exContainer.outerHeight) {
                ...
                log.debug("height: biComponent is smaller than container or equal OR container doesn't have vertical scroll bars");
                ...
                log.debug("height: biComponent is larger than container");

                top = exContainer.scrollTop -
                    (exContainer.scrollTop < exContainer.padding.top ? 0 : exContainer.padding.top);
                ...
                if (exContainer.scrollTop + exContainer.outerHeight >= exContainer.scrollHeight - exContainer.padding.bottom) {
                    ...
                    log.debug("height: scrolled to the end");
		...
		_applyScaleTransform: function() {
			...
			var scale = 'scale(' + this.scaleFactor + ")",
				origin = "top left",
				css = {
