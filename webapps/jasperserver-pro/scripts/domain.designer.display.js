...
dd.display = {
    ...
    getTreeModel: function(tree) {
        ...
        for (var i = 0; i< childNodes.length; i++) {
            if (childNodes[i]._markAsDeleted !== true) {
                domain.unescapeTree(childNodes[i]);
                childs.push(childNodes[i]);
            }
        }

        return Object.toJSON(childs);
    },
    ...
    initDragAndDrop: function() {
       ...
       var onDrop = function(dragged, dropped, event) {
           ...
           if (sourceTree === this.viewTree) {
               nodes = this.viewTree.selectedNodes;
    ...
    viewTreeLoadCallback: function() {
        ...
        this.updateButtonsState();
        this.setOriginalItemId(this.viewTree.getRootNode());
    },

    getFullNodePath: function(node) {
        var currentNode = node,
            pathParts = [currentNode.param.extra.originalItemId];

        while(currentNode = currentNode.parent) { // jshint ignore: line
            if (currentNode.param.extra) {
                pathParts.push(currentNode.param.extra.originalItemId);
            }
        }
    ...
    setOriginalItemId: function(node) {
        if (node.param.extra) {
            node.param.extra.originalItemId = node.param.extra.itemId;
        }

        if (node.childs) {
            _.each(node.childs, function(childNode) {
                dd_display.setOriginalItemId(childNode);
            });
        }
    },

    getItemTypeNodesRecursively: function(node) {
        var childNodes = [];

        if (node.param.type === "ItemType") {
            childNodes.push(node);
        }

        if (node.childs && node.childs.length) {
            _.each(node.childs, function(child) {
                childNodes = childNodes.concat(dd_display.getItemTypeNodesRecursively(child));
            });
        }
    ...
    setDontWarnOfDeleteItems: function(dontWarnOfDeleteItems) {
        this.dontWarnOfDeleteItems = false;//dontWarnOfDeleteItems;
    },
    ...
    deleteItemsFromViewTree: function(items, successDeleteTreeItemsCallback) {
        _.each(items, function(item) {
            item._markAsDeleted = true;
        });

        var fieldsWithDependentResources = [];

        var proceedWithDelete = function() {
            dd.setUnsavedChangesPresent(true);
        ...
        domain.designer.checkDesign(_.bind(function(response) {
            _.each(items, function(item) {
                item._markAsDeleted = false;
            });

            if (response === "success" || !response.errorMessage) {
                proceedWithDelete.call(this);
            } else {
                var usedResources = domainUtil.getUsedResourcesList(response.errorMessage, domain._messages['ITEM_BEING_USED_BY_RESOURCE']),
                    selectedFields = [];

                _.each(items, function(item) {
                    selectedFields = selectedFields.concat(dd_display.getItemTypeNodesRecursively(item));
                });

                selectedFields = _.uniq(_.map(selectedFields, dd_display.getFullNodePath));

                usedResources = _.filter(usedResources, function(resource) {
                    return _.indexOf(selectedFields, resource.field) !== -1;
                });

                if(usedResources.length || domainUtil.indexOfNoAccessMessage(response.errorMessage, domain._messages['resourcesWithNoAccess']) > -1) {
                    jQuery("#dependentResourcesConfirmMessage").html(domainUtil.getUsedResourcesFormattedMessage(
                        usedResources,
                    ...
                    dd_display.makeConfirmDialogSizeable();

                    domain.confirmDialog.show(
                        "dependentResourcesConfirmMessage",
                        function() {
                            proceedWithDelete.call(self);
                            dd_display.makeConfirmDialogNotSizeable();
                        },
                        function() {
                            dd_display.makeConfirmDialogNotSizeable();
                        });
    ...
    makeConfirmDialogSizeable: function() {
        jQuery(domain.confirmDialog._dialog)
            .addClass("detail")
            .addClass("noMaxWidth")
            .addClass("sizeable")
            .find(".sizer").show();
    ...
    makeConfirmDialogNotSizeable: function() {
        jQuery(domain.confirmDialog._dialog)
            .removeClass("detail")
            .removeClass("noMaxWidth")
            .removeClass("sizeable")
...
dd_display.propertiesEditor = {
    ...
    _getGetValueLabelPairForObject: function(key) {
        return {
            'value':key,
            'label':key
//            'label':domain.getMessage(key).replace(/&nbsp;/g,' ') // TODO: turn on support for localized function labels when adhoc will support them
        };
    },

    // Aggregation functions setting
    _initAggregations: function() {
        var sortSummaries = function(fun){
            // set None to the top of list
            return fun.value === "None" ? "AAA" : fun.value;
        };
        this._defaultAgg['int'] = this._defaultAgg['dec'] = _.sortBy(this.numberAggFunctionArray.collect(this._getGetValueLabelPairForObject), sortSummaries);
        this._defaultAgg['NaN'] = _.sortBy(this.nanAggFunctionArray.collect(this._getGetValueLabelPairForObject), sortSummaries);
        this._defaultAgg['date'] = this._defaultAgg['timestamp'] = this._defaultAgg['time'] = _.sortBy(this.dateAggFunctionArray.collect(this._getGetValueLabelPairForObject), sortSummaries);
    },
    ...
    _edit: function() {
        ...
        this._initialParamState = _.clone(this._node.param.extra);
    },
    ...
    _submitEdit: function() {
        ...
        if (this._validate()) {
            ...
            if (result) {
                var checkDesignResponse;

                domain.designer.checkDesign(
                    function(response) { checkDesignResponse = response; },
                    { synchronous: true });

                if (checkDesignResponse === "success" || !checkDesignResponse.errorMessage) {
                    callback(result);
                    return true;
                } else {
                    var usedResources = domainUtil.getUsedResourcesList(checkDesignResponse.errorMessage, domain._messages['ITEM_BEING_USED_BY_RESOURCE']),
                        affectedFields = _.uniq(_.map(dd_display.getItemTypeNodesRecursively(this._node), dd_display.getFullNodePath));

                    usedResources = _.filter(usedResources, function(resource) {
                        return _.indexOf(affectedFields, resource.field) !== -1;
                    });

                    if(usedResources.length || domainUtil.indexOfNoAccessMessage(checkDesignResponse.errorMessage, domain._messages['resourcesWithNoAccess']) > -1) {
                        jQuery("#dependentResourcesConfirmMessage").html(domainUtil.getUsedResourcesFormattedMessage(
                            usedResources,
                            checkDesignResponse.errorMessage,
                            {
                                "resource.label": domain._messages["resource.label"],
                                "field.label": domain._messages["field.label"],
                                "ITEM_BEING_USED_BY_RESOURCE": domain._messages['ITEM_BEING_USED_BY_RESOURCE'],
                        ...
                        domain.confirmDialog.show("dependentResourcesConfirmMessage",
                            function() {
                                dd_display.makeConfirmDialogNotSizeable();
                                callback(result);
                            },
                            function() {
                                dd_display.makeConfirmDialogNotSizeable();
                                factoryElem.submitChanges(self._node, self._initialParamState, callback);
                                factoryElem.view.bind(self)(self._node);
                                factoryElem.edit.bind(self)(self._node);
                            });
...
propsEditor.dateAggFunctionArray = [
    "None",
    "Max",
    "Min",
    "Mode",
...
// 2014-04-22 thorick
//            Aggregate 'Mode' should be available to all datatypes not just numeric
//
...
propsEditor.nodeTypeFactory = {
    'ItemGroupType': {
        ...
        validate: function(onlyHideErrors) {
            ...
            return ValidationModule.validateLegacy([
                ...
                {validator: dd.itemKeyValidator.bind(this, "itemNameKey.invalid"), element: $('itemNameKey')},
                {validator: dd.escapeJsonValidator.bind(this), element: $('itemDescriptionKey')},
                {validator: dd.itemKeyValidator.bind(this, "itemDescriptionKey.invalid"), element: $('itemDescriptionKey')},
                {validator: dd.emptyValidator.bind(this), element: $('itemID')},
                {validator: dd.itemIdValidator.bind(this, "id.invalid"), element: $('itemID')},
                {validator: dd.itemIdNotStartFromNumberValidator.bind(this), element: $('itemID')},
    ...
    'ItemType': {
        view: function(node) {
            ...
            var extra = node.param.extra;
            var emptyMessage = domain.getMessage('none');
            $('itemName').writeAttribute('readonly', 'readonly').value = extra.label ? xssUtil.unescape(extra.label) : emptyMessage;
            $('itemID').writeAttribute('readonly', 'readonly').value = xssUtil.unescape(extra.itemId);
            $('itemDescription').writeAttribute('readonly', 'readonly').value = extra.descr ? xssUtil.unescape(extra.descr) : emptyMessage;
            $('itemNameKey').writeAttribute('readonly', 'readonly').value = extra.labelId ? xssUtil.unescape(extra.labelId) : emptyMessage;
            $('itemDescriptionKey').writeAttribute('readonly', 'readonly').value = extra.descrId ? xssUtil.unescape(extra.descrId) : emptyMessage;
            ...
            var buildOptions = function(propName, option, index) {
                ...
                } else if (propName == 'defaultAgg') {
                    opt['selected'] =  opt.value == (isNumeric ? 'Sum' : 'CountAll');
                } else{
        ...
        submitChanges: function(node, values) {
            node.changeName(values.label ? values.label : values.itemId);
            $H(values).each(function(pair) {
                node.param.extra[pair.key] = pair.value;
            });
    ...
    'level': {
        ...
        validate: function() {
            return ValidationModule.validateLegacy([
                ...
                {validator: dd.itemIdValidator.bind(this, "id.invalid"), element: $('resourceItemId')},
                {validator: dd.itemIdNotStartFromNumberValidator.bind(this), element: $('resourceItemId')}
            ]);
        },
