...
dd.derivedTables = {
    ...
    getSelectedModel: function() {
        var rootNode = this.itemsTree.getRootNode();
        domain.unescapeTree(rootNode);
        return Object.toJSON(rootNode.childs);
    },
    ...
    saveTable: function(dataSource, queryId, query, selectedFields, editedNode) {
        ...
        selectedFields.each(function(selectedField) {
            ...
            var id = metanode.id + '.' + selectedField.itemId;

            var metafield = {
                id: id,
                label: selectedField.fieldDbName,
                type: 'item',
                uri: metanode.uri + '/' + id
            };
            ...
            metafield.extra = {
                ...
                dataSource: dataSource,
                datasourceId: dataSource,
                itemId: selectedField.itemId,
                originalId: metanode.extra.originalId + '.' + selectedField.itemId,
                fieldDbName: selectedField.fieldDbName,
                repoId: '',
    ...
    queryIdInvalidValidator: function(value) {
        return {
            isValid: dd.itemIdValidator(null, value).isValid && value.indexOf(".") < 0,
    ...
    queryExpressionValidator: function(value) {
        var isValid = false,
            errMessage = domain.getMessage('expression.empty');

        if (value.match(dd.derivedTableQueryRegex)) {
            isValid = true;
        } else {
    ...
    insertNodeToQuery: function(node) {
        ...
        if (!node.isParent()) {
            text = node.param.extra.fieldDbName;
        }
        ...
        if (domain.elementClicked(element, '#' + this.RUN_QUERY_BUTTON_ID)) {
            ...
            var callback = function(json) {
                if (json.error) {
                    ...
                    $(this.ERROR_REPORT_ID).update(xssUtil.escape(json.error));
                    ...
                    var options = this.queryResult.result.collect(function(option) {
                        var selected = selectAll ? true : selectedFields.indexOf(option.itemId) > -1;
                        return {value: option.itemId, label: option.fieldDbName, selected: selected};
                    }, this);
            ...
            var valid = ValidationModule.validateLegacy([{validator: this.queryExpressionValidator, element: this.expressionInput}]);
    ...
    saveTableClickHandler: function(element) {
        if (domain.elementClicked(element, '#' + this.SAVE_TABLE_BUTTON_ID)) {
            var valid = ValidationModule.validateLegacy([
            ...
            if (valid) {
                ...
                var selectedFields = this.queryResult.result.findAll(function(field) {
                    return selectedFieldIds.indexOf(field.itemId) >= 0;
                }.bind(this));
