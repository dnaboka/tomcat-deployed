...
var localContext = {
    ...
    SESSION_ATTRIBUTE_MISSING_EXCEPTION : "sessionAttributeMissingException",
    ...
    typeToView : {
        'com.jaspersoft.ji.adhoc.AdhocReportUnit' : 'viewAdhocReportFlow&viewAsDashboardFrame=true&dashboardViewer=true&reportUnit=',
        'com.jaspersoft.jasperserver.api.metadata.jasperreports.domain.ReportUnit' : 'viewReportFlow&viewAsDashboardFrame=true&dashboardViewer=true&reportUnit=',
        'com.jaspersoft.ji.adhoc.DashboardResource' : 'dashboardRuntimeFlow&viewAsDashboardFrame=true&dashboardViewer=true&dashboardResource='
    },
    frameTypes : [],
    framesByType : [],
    ...
    initAll : function(){
        ...
        webHelpModule.setCurrentContext("legacy_dashboard");

        this._updateAllStates();
        this.initRepoTree();
        this.initSpecialContentTree();
        this.initContentFrames();
     ...
     */
    initContentFrames: function() {
        var self = this;

        _.each(this.framesByType[this.CONTENT_FRAME], function(contentFrame) {
            var src = self._constructIframeSrc(contentFrame),
                frame = self._getIFrameByName(contentFrame.name);

            $(frame).observe("load", function() {
                self._updateIFrameLoadingStatus(self.OVERLAY_PREFIX + contentFrame.name, contentFrame.hasScrollBars ? "yes" : "no");
            });

            self.loadIframe(frame, src, contentFrame.resourceType === self.CUSTOM_RESOURCE_TYPE);
        });
    },
    ...
    loadIframe: function(iframe, url, isExternalUrl) {
        ...
        if (isExternalUrl) {
            var contentFrameName = this._getIFrameNameFromIFrame(iframe),
            ...
            this.customUrlLoadingTimeouts[contentFrameName] = setTimeout(function() {
                ...
                $div
                    .html(self.DASHBOARD_CUSTOM_URL_ERROR.replace("\n", "<br>"))
    ...
    _constructIframeSrc: function(contentFrame) {
        var isCustom = contentFrame.resourceType === this.CUSTOM_RESOURCE_TYPE,
            urlContext= isCustom ? '' : __jrsConfigs__.contextPath,
            iframeSrc = urlContext + contentFrame.source + contentFrame.uri;

        if (contentFrame.source.indexOf("http://") !== 0) {
    ...
    initControlsController: function() {
        this.controlsController = new JRS.Controls.DashboardController({
            reportUri : "/dashboard/designer/" + clientKey,
            viewModel : new JRS.Controls.DashboardViewModel()
        });
    ...
    initPreventBrowserSelection : function(domObject){
        ...
        if(object){
            if(isIE()){
                ...
                jQuery(domObject).on('onselectstart', function (e) {
    ...
    _updateIFramesSRCParam : function(contentFrame){
        ...
        if (contentFrame) {
            ...
            dashboardViewer_parm = (contentFrame.source.indexOf(localContext.DASHBOARD_VIEWER_PARAM) > -1)? "" : localContext.DASHBOARD_VIEWER_PARAM;
            rootSRC = removeTrailingSlash(contextPath + contentFrame.source + contentFrame.uri + fid_parm + dashboardViewer_parm);

            //update all input control params
            queryString = "";

            localContext.controlFrames.each(function(controlFrame) {
                var paramName = contentFrame.params ? contentFrame.params[controlFrame.paramName] : undefined;
                if (paramName) {
            ...
            if (queryString) {
                ...
                this.loadIframe(iFrame, rootSRC + queryString, contentFrame.resourceType === this.CUSTOM_RESOURCE_TYPE);
            }
        }
    },
    ...
    _getSelectedValuesForControlFrame : function(frame){
        var selection = this.controlsController.getViewModel().get("selection")[frame.paramName];
        return selection !== undefined ? selection : frame.paramValue;
    },
    ...
    initDashboardGeneralEvents : function(){
        ...
        jQuery('#customURL').on("click", "#customURLCancel", function(){
        ...
        jQuery('#customURL').on("click", "#customURLOk", function() {
    ...
    toggleScrollBarsOnClient : function(iFrame, show){
        if(iFrame){
            ...
            typeof($(iFrameObj).setStyle) === "function" && $(iFrameObj).setStyle(style);
        }
    },
    ...
    _isCustomFrame: function(frameObject, frameNameResolver) {
        var frameName = frameNameResolver(frameObject),
            contentFrames = localContext.contentFrames,
            currentFrame;

        if (!_.isEmpty(contentFrames)){
            currentFrame = _.find(contentFrames, function(frame) {
                return frame.name === frameName;
            });
        }

        return currentFrame && currentFrame.resourceType === localContext.CUSTOM_RESOURCE_TYPE;
    },
        ...
        if(this.currentEditingTextFrame){
            ...
            if (overlay) {
                ...
                if (oldText && replacementText) {
                    var escapedText = xssUtil.escape($(replacementText).value);
                    $(oldText).update(escapedText);
                    var fontSize = $(this.currentEditingTextFrame).getStyle("fontSize");
                    var newFrameWidth = this._getTextFrameMinimumWidthByLabel(frame, escapedText, fontSize);
                    $(this.currentEditingTextFrame).setStyle({
                        width : newFrameWidth + "px"
                    });
                    $(this.currentEditingTextFrame).removeClassName("editMode");
                    //now send to server
                    this.notifyServerUpdateText(frameName, escapedText, newFrameWidth);
                }
            }
        }
        ...
        this.selectedFrameOverlays.each(function(overlay){
            if(overlay){
                ...
                if (frameType === this.CONTROL_FRAME) {
                    var parentContainer = $(overlay).up("DIV");
                    if (parentContainer) {
                        var width = parseInt(jQuery(parentContainer).css("width"), 10);
                        var height = $(parentContainer).offsetHeight;
                        var object = {
                            frameName : name,
                            frameWidth : width,
    ...
    _prepareCustomURLPromptControls : function(frame){
        if (localContext.controlFrames.length > 0) {
            ...
            localContext.controlFrames.each(function(control, index){
                ...
                $(checkboxLabel).update(xssUtil.escape(control.label));
    ...
    _processCustomURLPromptInput : function(){
        ...
        if (customURLIFrame) {
            ...
            this.loadIframe(customURLIFrame, checkURLProtocol(newSrcURL, true), true);
    ...
    _getTextFrameMinimumWidth : function(frame){
        if (this.fontSizeTester) {
            ...
            $(this.fontSizeTester).update(xssUtil.escape(textValue));
    ...
    _getTextFrameMinimumWidthByLabel : function(frame, label, fontSize){
        if (this.fontSizeTester) {
            ...
            $(this.fontSizeTester).update(xssUtil.escape(label));
            return (this.fontSizeTester.offsetWidth + this.hackPadding);
        }else{
            throw("font size tester is not available!!!");
        }
    ...
    _purgeDeletedInputControlTextFrames : function(){
        ...
        localContext.textFrames.each(function(textFrame, index){
            ...
            if(inputControlRefName && inputControlRefName != "null"){
                var found = false;
    ...
    addContentFrame : function(event){	
        ...
        if(type != null && uri != null && name != null ) {
            ...
            if (this.shouldReplace && this.replacingFrameName) {
                ...
                if (iFrame) {
                    this.loadIframe(iFrame, urlContext + localContext.nextFrameSrc + currentPath, type === this.CUSTOM_RESOURCE_TYPE);
                    //update tool-tip
    ...
    _createDomForFrame : function(isCustom){
        ...
        if(!isCustom){
        	this.loadIframe(newIFrame, urlContext + localContext.nextFrameSrc + localContext._getCurrentReportPath() + '&fid=' + fid, false);
...
//                $("presentation").dispatchEvent(me);
    ...
    notifyServerAddContentFrame : function(options, event){
        ...
        var errorHandler = function(ajaxAgent) {
            //TODO: This condition will be removed after dashbord specific session attr will be serializable
            if (!(ajaxAgent.response && ajaxAgent.response.indexOf(localContext.SESSION_ATTRIBUTE_MISSING_EXCEPTION) > -1)) {
                localContext._decrementUnansweredAddRequests();
    ...
    notifyServerSaveDashboard : function(folder, name, desc, overwriteOK){
        ...
        var callback = function(){
            ...
            this._updateState();
            jQuery("#canvas .header .title").text(localContext.dashboardName);
            deferred.resolve();
        }.bind(localContext);
