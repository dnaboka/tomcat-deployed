    ...
    _checkDesignCallback: function(json) {
        if ('success' === json) {
            ...
            var usedResources = domainUtil.getUsedResourcesList(json.errorMessage, domain._messages['ITEM_BEING_USED_BY_RESOURCE']),
                errorMessage = (usedResources.length || domainUtil.indexOfNoAccessMessage(json.errorMessage, domain._messages['resourcesWithNoAccess']) > -1)
                    ? domainUtil.getUsedResourcesFormattedMessage(usedResources, json.errorMessage, {
