...
/* global orgModule, layoutModule, dynamicList, RegExpRepresenter,
    ValidationModule, matchAny, dialogs, invokeServerAction, invokeClientAction,
    primaryNavModule, invokeOrgAction, matchAny, encodeUriParameter
*/
    ...
    initialize: function(options) {
        ...
        orgModule.entityList._createEntityItem = function(value) {
            ...
            item.processTemplate = function(element) {
                ...
                id.update(xssUtil.escape(this.getValue().id));
                name.update(xssUtil.escape(this.getValue().name));
                
                var subTenantCount = this.getValue().subTenantCount;
                subTenantCount = subTenantCount || 0;
                org.update(xssUtil.escape(orgModule.getMessage("MT_SUB_TENANT_COUNT", {count: subTenantCount})));
...
orgModule.orgManager.addDialog = {
    ...
    _clickButtonHandler: function(event) {
        ...
        if (button == this.addBtn) {
            var that = this;
            var createdCallback = function(event){
...
orgModule.orgManager.properties = {
    ...
    initialize: function(options) {
        ...
        orgModule.properties.initialize(options._.extend(options, {
            ...
            attributes: {
                context: {
                    urlGETTemplate: "rest_v2/attributes?_embedded=permission&includeInherited=true{{#id}}&holder=tenant:/{{id}}{{/id}}&group=custom&excludeGroup=serverSettings",
                    urlPUTTemplate: "rest_v2{{#id}}/organizations/{{id}}{{/id}}/attributes?_embedded=permission"
                }
            },
            showAssigned: false
        }));
        ...
        orgModule.properties._deleteEntity = function() {
            invokeClientAction("delete", {entity: this._value, entityEvent: !!orgModule.entityList.getSelectedEntities().length});
        };
        ...
        orgModule.properties.isChanged = function() {
            ...
            return this.isEditMode && (isAttributesChanged || oldOrg.name != org.name || oldOrg.alias != org.alias
